...
for language in "${eval_languages[@]}"; do
    ...
    if python -m $search_python_script --index "$index_file" --topics "mlep-meta-bm25-$variant/mlep-bm25-$language/queries.tsv" --output "output/mlep-meta-bm25-$variant/mlep-bm25-$language.txt" --language "$lowercase_language" --bm25; then
    ...
    if python -m $eval_python_script -c -M "$k" "mlep-meta-bm25-$variant/mlep-bm25-$language/qrels.txt" "output/mlep-meta-bm25-$variant/mlep-bm25-$language.txt" > "output/mlep-meta-bm25-$variant/ranking_metric_$language.txt" 2>&1; then
