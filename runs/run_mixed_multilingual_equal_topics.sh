...
# Read user input for sample size
read -p "Enter the sample size for queries: " sample_size
...
# Index file
index_file="indexes/mlep-meta-bm25-$variant/"
...
for language in "${eval_languages[@]}"; do
    ...
    if python -m $search_python_script --index "$index_file" --topics "mlep-meta-bm25-full-equal-topics/${sample_size}topics/mlep-bm25-$language/queries.tsv" --output "output/mlep-meta-bm25-full-equal-topics/${sample_size}topics/mlep-bm25-$language.txt" --bm25; then
    ...
    if python -m $eval_python_script -c -M "$k" "mlep-meta-bm25-full-equal-topics/${sample_size}topics/mlep-bm25-$language/qrels.txt" "output/mlep-meta-bm25-full-equal-topics/${sample_size}topics/mlep-bm25-$language.txt" > "output/mlep-meta-bm25-full-equal-topics/${sample_size}topics/ranking_metric_$language.txt" 2>&1; then
