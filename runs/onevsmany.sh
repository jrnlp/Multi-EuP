...
for language in "${selected_languages[@]}"; do
    ...
    if [[ "$model" == "bm25" ]]; then
        ...
        python_command="python -m pyserini.search.lucene --index /lt/scratch/jinruiy/MLEP-data/pyserini-data/indexs/$ir_task --topics mlep-meta-bm25-splited/$task_folder/mlep-bm25-$language/queries.test.tsv --output output/mlep-meta-bm25-splited/$task_folder/whitespace/mlep-bm25-$language-test.txt --bm25"
